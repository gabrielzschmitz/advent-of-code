/**
 * @file part2.cpp
 * @brief Pipe Maze - Calculating Enclosed Area within the Loop
 *
 ...
 * that contains the animal (starting at S) and determine the number of tiles
 * enclosed by the loop, considering areas between pipes as well.
 *
 * Pipe Types:
 ...
 * forming a continuous loop. Calculate the number of tiles enclosed by the
 * loop, considering spaces between pipes as part of the loop.
 *
 * Example Input:
 * ...........
 * .S-------7.
...
int main(int argc, char** argv) {
  ...
  if (shape_area < 0) { shape_area = shape_area * -1; }
  int innerTiles = interiorPoints(shape_area, distance);

  cout << "Inner tiles = " << innerTiles << endl;
