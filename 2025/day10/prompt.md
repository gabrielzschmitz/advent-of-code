...
initialization procedure was eaten by a [Shiba
Inu](https://en.wikipedia.org/wiki/Shiba_Inu). All that remains of the manual
are some indicator light diagrams, button wiring schematics, and [joltage](3)
...
To start a machine, its _indicator lights_ must match those shown in the
diagram, where `.` means _off_ and `#` means _on_. The machine has the number
of indicator lights shown, but its indicator lights are all _initially off_.
...
simultaneously configure the first light to be off, the second light to be on,
the third to be on, and the fourth to be off.

You can _toggle_ the state of indicator lights by pushing any of the listed
_buttons_. Each button lists which indicator lights it toggles, where `0`
means the first light, `1` means the second light, and so on. When you push a
...
you will need to determine the _fewest total presses_ required to correctly
...
However, the fewest button presses required is `_2_`. One way to do this is by
...
The fewest presses required to correctly configure it is `_2_`; one way to do
...
lights on all of the machines is `2` \+ `3` \+ `2` = `_7_`.

Analyze each machine's indicator light diagram and button wiring schematics.
_What is the fewest button presses required to correctly configure the
indicator lights on all of the machines?_
...
Each machine needs to be configured to _exactly the specified joltage levels_
...
The machines each have a set of _numeric counters_ tracking its joltage
levels, one counter per joltage requirement. The counters are all _initially
set to zero_.
...
`0` means the first counter, `1` means the second counter, and so on. When you
push a button, each listed counter is _increased by`1`_.
...
the _fewest total presses_ required to correctly configure each machine's
...
Configuring the first machine's counters requires a minimum of `_10_` button
presses. One way to do this is by pressing `(3)` once, `(1,3)` three times,
`(2,3)` three times, `(0,2)` once, and `(0,1)` twice.

Configuring the second machine's counters requires a minimum of `_12_` button
presses. One way to do this is by pressing `(0,2,3,4)` twice, `(2,3)` five
times, and `(0,1,2)` five times.

Configuring the third machine's counters requires a minimum of `_11_` button
...
level counters on all of the machines is `10` \+ `12` \+ `11` = `_33_`.

Analyze each machine's joltage requirements and button wiring schematics.
_What is the fewest button presses required to correctly configure the joltage
level counters on all of the machines?_
