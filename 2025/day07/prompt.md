...
manifold - it's a _quantum tachyon manifold_.

With a quantum tachyon manifold, only a _single tachyon particle_ is sent
through the manifold. A tachyon particle takes _both_ the left and right path
...
actually _time itself_ which splits. In one timeline, the particle went left,
and in the other timeline, the particle went right.

To fix the manifold, what you really need to know is the _number of timelines_
...
manifold diagram. _In total, how many different timelines would a single
tachyon particle end up on?_
