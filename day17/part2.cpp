/**
 * @file part2.cpp
 * @brief Clumsy Crucible - Minimize Heat Loss (Ultra Crucibles)
 *
 * Determine the least heat loss incurred by directing an ultra crucible from
 * the lava pool (starting point) to the machine parts factory (destination) on
 * Gear Island. Ultra crucibles are even more difficult to steer than normal
 * crucibles, requiring a minimum of four blocks to turn and a maximum of ten
 * consecutive blocks before turning. Minimize heat loss by choosing a route
 ...
 * ultra crucible from the lava pool to the machine parts factory. Ultra
 * crucibles require a minimum of four blocks to turn and a maximum of ten
 * consecutive blocks before turning.
 *
    ...
    if (currentStep >= 4 ||
      ...
      for (const auto& direction :
        ...
        if ((nDirectionRows != currentDirectionRow ||
          ...
          int nColums = currentColumn + nDirectionColumns;
          if (0 <= nRows && nRows < rows && 0 <= nColums && nColums < columns) {
            priorityQueue.push({currentHeatLoss + heatMap[nRows][nColums],
                                nRows, nColums, nDirectionRows,
                                nDirectionColumns, 1});
          }
        }
      }
