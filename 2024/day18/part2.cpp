...
std::pair<int64_t, int64_t> findBlockingCoordinate(
  const std::vector<std::pair<int64_t, int64_t>>& coordinates) {
  std::unordered_set<std::string> corrupted;

  for (const auto& coord : coordinates) {
    corrupted.insert(std::to_string(coord.first) + "," +
                     std::to_string(coord.second));
    if (!checkPathExists(corrupted)) { return coord; }
  }
...
int main(int argc, char* argv[]) {
  ...
  std::pair<std::int64_t, std::int64_t> total =
    findBlockingCoordinate(coordinates);

  std::cout << "ANSWER: " << total.first << "," << total.second << std::endl;
  return 0;
}
